...
Prefix caching requires an **exact byte-level match**: cached reads cost ≈10% of base input, writes carry a 25–100% premium, so one mutated byte *before the current turn* invalidates
everything after it and re-pays the full prompt every call ([prompt-caching-architectural-discipline](https://agentpatterns.ai/context-engineering/prompt-caching-architectural-discipline/);
...
3. **Run the detectors** in [`checks.md`](checks.md) (PC-1…PC-9): prefix volatility, tool ordering,
   tool mutation, assembly order, per-call personalization, mid-session switch, monitoring, fleet
   per-machine context, economics. Load it when auditing.
...
| Med | PC-7 no monitoring | no `usage` inspection | (silent) | log `cache_read` vs `cache_creation` per session |
...
```
Severity: **High** = prefix bust every call (full re-pay); **Medium** = per-switch / per-machine
bust or a silent monitoring gap; **Low** = economics/TTL tuning or least-cost cleanup.
...
- **Misses are silent** — always check `cache_read` vs `cache_creation`; a creation spike mid-session
  is a prefix regression, not an error.
