...
- **Audit by run shape, not feature checklist.** Identity-propagation and per-source attribution are
  overhead on a single-agent / minimal-source run — the corpus calls them so. **Suppress** OBS-5 and
  OBS-7 with no fan-out / few sources; do not false-positive (see Scope).
...
audit). **Run-shape suppression:** single-agent → skip OBS-5/6 (no fan-out to
correlate); minimal-source → skip OBS-7. Legibility + trail checks (OBS-1,2,3,9,10) always apply.
...
## Related / pairing
- Seam with **`audit-verification-gates`** (#24): this checks the gate is *wired and trace-coupled*;
...
- **Audit by run shape:** suppress identity/attribution checks where the corpus says they are
  overhead; do not manufacture findings.
