...
the "why") / **Fix** (cited lesson = the "how"). All detection is read-only. Apply the run-shape
suppression in the SKILL before flagging OBS-5/6 (need fan-out) and OBS-7 (need multiple sources).
...
- **Fix:** enable telemetry + an exporter and an export interval ([leaving-a-trail](https://learn.agentpatterns.ai/observability/leaving-a-trail/)). Auditor: `otel-instrumentation-setup`.
...
- **Fix:** add `maxTurns` to the agent frontmatter (and a cost budget) ([breaking-the-loop](https://learn.agentpatterns.ai/observability/breaking-the-loop/)). Auditor: `loop-detection-circuit-breakers`.
...
- **Fix:** add the `PostToolUse(Edit|Write)` edit-count hook + doom-loop detector ([breaking-the-loop](https://learn.agentpatterns.ai/observability/breaking-the-loop/)). Auditor: `loop-detection-circuit-breakers`.
...
- **Fix:** move the ceiling to `maxTurns` / a deterministic hook; keep the instruction only as a
  secondary error-rate signal the runtime can't detect ([breaking-the-loop](https://learn.agentpatterns.ai/observability/breaking-the-loop/)). Auditor: `loop-detection-circuit-breakers`.
...
- **Fix:** emit `agent_id`/`parent_agent_id` on header and span; slice metrics by `agent.name`, drill
  by `agent_id` ([one-id-across-the-trace](https://learn.agentpatterns.ai/observability/one-id-across-the-trace/)). Auditor: `trace-context-subagent-correlation`.
...
- **Fix:** lift the shell-out into an instrumented MCP tool, or wrap it with an explicit
  `x-claude-code-agent-id` header ([one-id-across-the-trace](https://learn.agentpatterns.ai/observability/one-id-across-the-trace/)). Auditor: `trace-context-subagent-correlation`.
...
- **Fix:** break context down per source; confirm the denominator covers input + output + cache
  ([attributing-the-context](https://learn.agentpatterns.ai/observability/attributing-the-context/)). Auditor: `context-token-cost-attribution`.
...
- **Fix:** wire a gate into CI and store its verdict on the run with the trace ID ([gates-that-catch-regressions](https://learn.agentpatterns.ai/observability/gates-that-catch-regressions/)). Auditor: `feedback-coupled-traces`.
...
- **Fix:** add a progress file read-at-start + commit-per-task; flip `feature-state` only
  post-verification ([leaving-a-trail](https://learn.agentpatterns.ai/observability/leaving-a-trail/)). Auditor: `trajectory-progress-logging`.
...
- **Fix:** wire a log/metric/browser MCP signal the agent can read; treat empty MCP results as
  unknown, not clear ([write-and-hope](https://learn.agentpatterns.ai/observability/write-and-hope/)). Auditor: `agent-self-verification-signals`.
