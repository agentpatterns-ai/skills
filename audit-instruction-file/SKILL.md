...
description: Audit a single agent instruction file (AGENTS.md, CLAUDE.md, .cursorrules, copilot-instructions.md, GEMINI.md, …) against context-engineering principles and recommend the smallest fix. Invoke when reviewing, writing, or debugging one instruction/memory file, or when an agent ignores documented rules within a single file. Skip when editing ordinary docs or source code, or when rules across two or more different files contradict each other (use conflicting-instruction-detector).
user-invocable: true
version: "0.1.0"
...
2. **Measure** each file:
   ...
   ```
   Also count `##`/`###` headings (section count) and the **normative statements**
