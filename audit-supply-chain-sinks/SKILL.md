...
4. **Recommend** the per-sink deterministic control (fix column in `checks.md`), preferring
   ...
   the model writing the sink string. Remediation:
   [the-output-is-untrusted-too](https://learn.agentpatterns.ai/security/the-output-is-untrusted-too/),
   [the-package-that-doesnt-exist](https://learn.agentpatterns.ai/security/the-package-that-doesnt-exist/).
...
## Findings
| Severity | Path | Sink | Why (source) | Fix (deterministic) |
|---|---|---|---|---|
| High | query-agent | SQL string-concat | CVE-2025-1793 SQLi class — model writes the WHERE clause [LLM05] | schema object (Pydantic) → parameterised `execute(sql, params)` |
| High | query-agent | ungated `uv add`  | slopsquatting: agent-named dep, no lockfile [arXiv:2406.10279] | `uv pip sync`/`npm ci` against committed lockfile; manifest change via reviewed PR |
