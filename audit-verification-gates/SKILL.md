...
|---|---|---|---|---|
| High | VG-1/2 | main agent | "Be thorough; check your work" + self-reported "all tests pass" — no hook, not state-anchored | Stop/PostToolUse hook that runs the suite on the branch and gates on exit code + a baseline/after ledger |
| High | VG-8 | grader.py | asserts the tool-call sequence, not the end-state | grade outcome: pytest exit code / schema state |
| High | VG-9 | red-green | agent may edit tests to pass them | lock tests in green ("unacceptable to edit tests"); confirm red first |
