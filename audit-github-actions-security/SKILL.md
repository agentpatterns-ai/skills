...
description: Audit GitHub Actions workflow files (.github/workflows/*.yml, composite/reusable actions) against the OWASP Top 10 CI/CD Security Risks (CICD-SEC-1..10) — untrusted ${{ github.event }} expanded into run-step shells (script injection), pull_request_target/workflow_run pwn-requests, third-party actions pinned to mutable tags instead of a full commit SHA, over-broad GITHUB_TOKEN permissions, long-lived secrets vs OIDC, missing fork-PR approval/flow-control, self-hosted runners exposed to untrusted code, ungoverned curl-pipe-bash third-party services, unsigned artifacts without provenance, and security-masking continue-on-error/debug-leak/visibility gaps. Invoke when reviewing, hardening, or writing a GitHub Actions workflow or CI/CD pipeline config, or vetting a workflow before merge. Skip when scanning an agent's own context files for literal hardcoded credentials (use audit-secret-exposure), auditing an agent harness's output/dependency-install sinks (use audit-supply-chain-sinks), or auditing an agent's private-data x untrusted-input x egress architecture (use audit-lethal-trifecta).
user-invocable: true
version: "0.1.0"
