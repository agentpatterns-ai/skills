...
- **Flags:** pure-read tools missing `readOnlyHint: true` **+ `idempotentHint: true`** (pure reads
  are idempotent by definition — the pair gives a harness the documented safe-retry path); a
...
- **Fix:** `readOnlyHint: true` + `idempotentHint: true` on pure reads
...
- **Flags:** stdio where shared state / multiple concurrent clients demand Streamable HTTP (or HTTP
  for a single local tool); a **locally-running** HTTP server that doesn't validate `Origin` / bind
...
- **Fix:** match transport to topology; validate `Origin`/bind localhost on locally-running HTTP;
  authenticate always. Remediation: [learn — what-a-server-exposes](https://learn.agentpatterns.ai/mcp-server-design/what-a-server-exposes/).
...
- **Fix:** sanitize stdio args at the boundary before any shell/exec.
  Remediation: [learn — what-a-server-exposes](https://learn.agentpatterns.ai/mcp-server-design/what-a-server-exposes/). **Route whole-harness exfiltration risk to `audit-lethal-trifecta`.** Remediation:
