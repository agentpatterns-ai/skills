...
- **Schemas don't sanitize input.** Unsanitized args reaching a shell/exec are a command-injection
  hole no richer schema closes, on any transport — the stdio SDK even runs commands when the local
  process fails to start ([mcp-server-design,
...
3. **Classify severity.** High = misroutes, misleads, or endangers the agent (wrong primitive,
   dishonest annotation, bare errors, bloated surface, **unsanitized shell-bound args — MS-9b**).
...
4. **Recommend the spec-correct fix per finding** (cite the rule); prefer the deterministic form
   ...
   trust and arg-injection *whole-harness* risk to `audit-lethal-trifecta`.
...
| High | MS-7 | server | 24 tools, no lazy-discovery plan | split server or defer behind search/read (target <15) | checks.md → MS-7 lesson URL |
...
- **`audit-lethal-trifecta`** — owns whole-harness exfiltration risk; this skill flags annotation-
  trust (MS-5) and arg-injection (MS-9b, High) as *design* defects and routes the architecture question there.
...
- **Schemas don't sanitize** — flag unsanitized args reaching a shell/exec as command injection, on
  any transport.
