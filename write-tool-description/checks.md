# write-tool-description — output requirements (WD-1…WD-12)
...
(cited corpus) / **How** (remediation lesson). They pair 1:1 with the detector
**`audit-tool-definition`** TD-* checks: this skill produces what that skill flags.
...
- **Require:** keep the description lean (~≤500 tokens — the minimum needed to prevent misuse, paid on
  every invocation); for MCP tools make each description self-contained (repeat domain context inline).
