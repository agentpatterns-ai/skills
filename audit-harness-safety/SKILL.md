...
description: Audit an agent harness's setup/architecture for blast-radius containment and recovery — OS-level sandbox + least-privilege scope, hard deny floors, reversibility/idempotency, a kill path, and consumption bounds — bounding the *damage* if a control fails. Invoke when hardening a harness for destructive-action safety — before granting write/Bash/network tools, running an agent headless or in CI, or when it touches the native filesystem, real credentials, shared branches, production, or paid APIs; also when reviewing reversibility/kill-path/cost posture. Skip when the question is data exfiltration via the lethal trifecta (use audit-lethal-trifecta) or instruction-prose attention/density (use audit-instruction-file).
user-invocable: true
version: "0.1.0"
...
- `path` (optional): an agent-config dir or repo. Default: scan the harness — `.claude/` (agent &
  ...
  caps, budgets, circuit breakers); CI/workflow files (`.github/workflows/*` `permissions:`, deploy
  sequences, branch/PR strategy).
...
(use `audit-instruction-file`); application source-code vulnerabilities (general security review); live
runtime monitoring (static config audit).
...
3. **Score severity** (table below). High = native write/exec with no sandbox and no deny floor, an
   irreversible action with no gate, or no kill path on a long-running/headless agent.
...
| HS-3 | No hard `deny` for `Bash(rm *)`/force-push/prod-deploy and secret files (`.env*`,`*.pem`,`*.key`,`id_rsa*`,`.aws/credentials`,`.ssh/**`) | High |
| HS-4 | Missing/wildcard `tools:` (inherits all) or GH Actions `permissions: write-all`/unscoped | High/Low |
...
|---|---|---|---|---|---|
| High | deploy-agent | HS-1,3,5,8 | unsandboxed prod deploy + `rm`, no undo, no stop | `--network none` + container; `deny:[Bash(rm *),Edit(.env*)]`; gate deploy; `maxTurns` | [sandboxing-and-blast-radius](https://learn.agentpatterns.ai/harness-engineering/sandboxing-and-blast-radius/) |
...
```
Severity: **High** = HS-1/3/4(wildcard)/5/8 live on native state. **Medium** = HS-2/6/9. **Low** = HS-4 (least-privilege tightening) / HS-10.
