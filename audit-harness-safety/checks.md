...
- **Why:** the deny rule is the floor the classifier "can't talk its way past" — even the framework's
  ...
  Deterministic auditor rule (`docs/security/safe-command-allowlisting.md`): deny read+edit of
...
- **Flags:** any agent or sub-agent with a **missing or wildcard `tools:`** field (inherits
  everything) or unscoped `permissions`; GitHub Actions `permissions: write-all` or unset.
- **Why:** damage is bounded by what you grant — scope four dimensions (tool / file / mode / repo) per
  ...
  Deterministic auditor rules (`docs/agent-readiness/audit-permissions-blast-radius.md`): explicit
  minimal `tools` list per sub-agent (no wildcard inherit); GH Actions `read-all` + per-step write.
- **Fix:** declare an explicit minimal `tools:` list; scope Actions to `read-all` with per-step write
  grants. Remediation: [bound-the-blast-radius](https://learn.agentpatterns.ai/security/bound-the-blast-radius/).
...
- **Why:** choose the undo before the do; rank each action on undo cost — external side effects have
  ...
  Auditor (`docs/workflows/human-in-the-loop.md`, `docs/agent-readiness/audit-confirmation-gate-logs.md`):
