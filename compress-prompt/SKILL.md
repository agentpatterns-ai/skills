...
then **lexical**. The seven-technique catalog — tables/bullets/rules over prose, negative
constraints, examples over descriptions, front-loading, structural compression — with when-to-use,
citations, and a worked before/after lives in [`techniques.md`](techniques.md): **load it before
compressing**; it is the one catalog (not restated here).
