...
- **Fix:** report "GEO does not apply — suppressed" with the reason; never propose fabricating or
  padding (each GEO lesson closes with a "when it doesn't help / don't game it" section).
...
- **Why:** the chunk's dominant signal is its opening text; preamble makes the embedding a diluted
  average of throat-clearing and answer. Anthropic's Contextual Retrieval cut retrieval failure up
  to **49%** by fixing chunk-opening text ([answer-first-writing](https://agentpatterns.ai/geo/answer-first-writing/);
...
- **Flags (deterministic):** an H2 section **<200 or >400 words** (~256–512 tokens), or a whole page
  **under ~200 words** (below the retrieval floor).
- **Why:** 256–512-token chunks retrieve best; too-short = thin context the LLM can't answer from,
  ...
  NVIDIA 2024: 0.648; [GEO paper](https://arxiv.org/abs/2311.09735)).
...
- **Flags (deterministic):** `/robots.txt` issues a `Disallow: /` that reaches a Tier-1 **retrieval**
  ...
  override. Also flag three-tier violations: training scrapers (`GPTBot`, `ClaudeBot`,
  `Google-Extended`, `Meta-ExternalAgent`) left **allowed**, or a non-compliant bot (`Bytespider`)
  "blocked" only in robots.txt with no WAF rule.
- **Why:** a blocked retrieval bot never fetches the page, so it can never be cited — this is a
  **live-fetch** block (not indexing-time hygiene like M1/M2), and **fatal to citability**. The
  correct policy is three-tier: **allow** retrieval / **disallow** training / **WAF-block**
...
- **Fix:** allow Tier-1 retrieval bots; disallow Tier-2 training scrapers; enforce non-compliant
  blocks at CDN/WAF — never rely on robots.txt for hard enforcement. **Severity High** (a blocked
