...
page-level chunking 0.648, 256–512-token chunks best for factoid). Also target site-root
...
## Findings
| Check | Where | Flag | Fix (→ lesson) |
|---|---|---|---|
| GEO-S1 buried answer | guide/setup.md#configuration | Opens "In this article we'll explore…" — preamble averages the chunk embedding | Lead with a 40–60-word direct answer (answer-first-atomic-pages) |
| GEO-S2 generic heading | guide/setup.md#overview | `## Overview` — zero discriminative signal, weak deep-link anchor | Rename to a concept-naming heading (answer-first-atomic-pages) |
| GEO-S3 length out of band | guide/setup.md | 2,040-word page, no H2 splits — diluted blended embedding | Split into 200–400-word atomic sections (answer-first-atomic-pages) |
| GEO-S4 non-atomic | guide/setup.md | One page covers install + auth + billing + SDK — five mechanisms | One concept per page (answer-first-atomic-pages) |
| GEO-D1 low assertion density | guide/setup.md#auth | "experts say this is significantly faster" — unattributed + anchorless | Numbers+units, named/dated source (assertion-density) |
| GEO-D2 no stat+source | guide/setup.md#billing | No statistic and no source link in a substantive H2 | Add ≥1 sourced stat with units (assertion-density) |
| GEO-M1 missing llms.txt | <site root> | No `/llms.txt` or `/llms-full.txt` | Publish curated index; note: navigation aid, NOT a ranking signal (machine-readable-corpora) |
...
```
Severity by retrieval impact: **High** = structural miss (buried answer, non-atomic page) that loses
the chunk entirely; **Medium** = substance gaps (low density, missing stat/source); **Low** =
machine-readable hygiene (llms.txt/schema), accruing at indexing time, not live fetch.
