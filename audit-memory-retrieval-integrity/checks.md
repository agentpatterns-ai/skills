...
- **Why:** SDK defaults read filesystem settings, global config, and auto-memory regardless of
  `settingSources`; each is a cross-tenant leak in a shared container, and session-keyed state carries
...
- **Fix:** restrict memory writes to user-authored / human-curated sources; attach a per-entry
  ...
  writes + egress co-exist; else **Medium**. Remediation:
  [learn — the-payload-that-waits](https://learn.agentpatterns.ai/security/the-payload-that-waits/).
