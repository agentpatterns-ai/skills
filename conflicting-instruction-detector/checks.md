...
- **Why:** this is the cross-type conflict models detect worst (~60% F1, ConInstruct — the
  ...
  ([agents-md-distributed-conventions](https://agentpatterns.ai/instructions/agents-md-distributed-conventions/)).
...
- **Flags:** two applicable rules in *different* **peer documents with no defined load order**
  prescribe different actions and **no precedence is stated** (e.g. two sibling rule files, two
  skills claiming the same trigger). Not necessarily a contradiction in content — an *unresolved
  ordering*. *Not this check:* docs in a defined load-order stack (user/project/local CLAUDE.md,
  CLAUDE.md vs a loaded SKILL.md) — layer-stack precedence is `audit-instruction-file`'s lane
  (CE-5; this skill's Scope defers it).
