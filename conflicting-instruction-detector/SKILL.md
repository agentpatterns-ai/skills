...
to ~60% F1 on cross-type conflicts ([ConInstruct, arXiv:2511.14342](https://arxiv.org/abs/2511.14342)),
so many flags are false positives — every flag is a **review candidate, never a confirmed conflict**,
and there is **no `--fix` / auto-resolve path**. Measured: models that *do* detect a conflict
overwhelmingly fail to surface it, silently resolving ~97.5% of the time (ConInstruct) — so it forces
the surface and hands the choice to a human.
...
3. **Overlap gate — run BEFORE ranking; the main false-positive filter.** For each candidate pair,
   ...
   **do not conflict**: route to *likely-not-a-conflict*, drop from the queue. Only pairs with a real
   overlapping input proceed. At ~60% F1 the dominant error is rules that look opposite but apply to
   disjoint scopes; this gate removes them.
