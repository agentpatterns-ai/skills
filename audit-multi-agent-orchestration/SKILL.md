...
description: Audit an existing multi-agent orchestration's coordination mechanics — typed vs prose handoffs, shared mid-run state, whether "done" is decided by a verifier that is a separate agent role from the producer, and loop bounds — and flag the structural smells that make multi-agent systems fail. Invoke when reviewing or hardening a harness that already runs more than one agent (orchestrator-worker, fan-out/synthesis, lead-and-teammates, generator-critic loops). Skip when deciding whether to go multi-agent at all or which topology (use architecture-committee), auditing the general completion-gating architecture — hooks, ledgers, graders, red-green anchoring (use audit-verification-gates; this skill covers only the multi-agent angle — the verifier's independence from the producer), auditing security legs / prompt-injection (use audit-lethal-trifecta), or a single instruction file's prose (use audit-instruction-file).
user-invocable: true
version: "0.1.0"
...
| MA-S2 | High | synthesis | `"\n".join(outputs)` | concatenation adds ~15x cost, no quality | reasoning merge: reconcile conflicts, weight reliability |
