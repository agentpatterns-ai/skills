...
- **Flags:** the merge step concatenates/aggregates worker outputs (`"\n".join(...)`, a vote, a
  summary) instead of evaluating reliability, reconciling conflicts, and producing a unified result.
...
- **Flags:** a gate exists but fails one of the four conditions — not independent of the producer
  ...
  no ground truth, or was **promoted from advisory to enforcing with no measured blocked precision**.
  Also flag bypass paths (direct file writes around the gate). Do **not** pass it as "has a gate."
...
- **Flags:** a fixed agent count baked into harness code, or no effort-scaling heuristics in the
  orchestrator's *prompt* — the most common failure is over-spawning on simple queries.
