...
|---|---|
| Anthropic / OpenAI key | `sk-ant-[A-Za-z0-9_-]+`, `sk-[A-Za-z0-9]{20,}`, `sk-live-…` |
...
- **Fix:** WIF / OIDC-federated short-lived token; **unset** any leftover key (don't blank); confirm
  with `ant auth status`. Remediation:
  [learn — keys-that-expire-in-minutes](https://learn.agentpatterns.ai/security/keys-that-expire-in-minutes/). **Severity Medium.**

### SE-5 — Secret / user data carried in a URL (query-string exfiltration channel)
- **Flags:** a tool/skill that constructs or follows URLs built from context, with credentials or
  user-specific data placeable in the query string, embedded resources auto-fetched, and
...
- **Flags:** `curl -H "Authorization: Bearer ghp_…"`, `api_key: sk-live-…`,
  ...
  no secret-scanner path rule (`betterleaks`/`trufflehog`) covering `.claude/skills/`, `skills/`.
